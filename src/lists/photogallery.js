import rozhdestvenskijChasVeselykhZhatejDomaUTaniSukhodoevoj from '../images/photogallery/2020/rozhdestvenskij-chas-veselykh-zhatej-doma-u-tani-sukhodoevoj.jpg'
import vystavkaOPtitse2020Zhuravle from '../images/photogallery/2020/vystavka-o-ptitse-2020--zhuravle.jpg'
import vystavkaProsmotrOChekhove from '../images/photogallery/2020/vystavka-prosmotr-o-chekhove.jpg'
import kruzhokBiblionikKJubilejuChekhova from '../images/photogallery/2020/kruzhok-biblionik-k-jubileju-chekhova.jpg'
import besedaIViktorinaOChekhoveDlyaTaniSukhodoevoj from '../images/photogallery/2020/beseda-i-viktorina-o-chekhove-dlya-tani-sukhodoevoj.jpg'
import podarennyeGazetyOtPanishevojNV from '../images/photogallery/2020/podarennye-gazety-ot-panishevoj-n-v.jpg'
import plakatKMezhdunarodnomyDnjuKnigodareniya from '../images/photogallery/2020/plakat-k-mezhdunarodnomy-dnju-knigodareniya.jpg'
import plakatKMezhdunarodnomyDnjuKnigodareniya2 from '../images/photogallery/2020/plakat-k-mezhdunarodnomy-dnju-knigodareniya-2.jpg'
import vystavkaAntarktidaKholodilnikZemli from '../images/photogallery/2020/vystavka-antarktida--kholodilnik-zemli.jpg'
import JulyaKhlebnikovaSKnigojVRukakh from '../images/photogallery/2020/Julya-Khlebnikova-s-knigoj-v-rukakh.jpg'

import semejnyjKlub29Marta from '../images/photogallery/2019/semejnyj-klub-29-marta.jpg'
import semejnyjKlub29Marta2 from '../images/photogallery/2019/semejnyj-klub-29-marta-2.jpg'
import chasZozhVperedPoDorogeZdoroviya from '../images/photogallery/2019/chas-zozh-vpered-po-doroge-zdoroviya.jpg'
import viktorinaNaDomuDlyaDevochkiSInvalidnostuj from '../images/photogallery/2019/viktorina-na-domu-dlya-devochki-s-invalidnostuj.jpg'
import vystavkaTeatrIBiblioteka from '../images/photogallery/2019/vystavka-teatr-i-biblioteka.jpg'
import detsadovtsyVMaskah from '../images/photogallery/2019/detsadovtsy-v-maskah-na-zanyatii-kruzhka-biblionik.jpg'
import bibliochuchelo from '../images/photogallery/2019/bibliochuchelo-dlya-biblionochi.jpg'
import bibliopiknik1Maya from '../images/photogallery/2019/bibliopiknik-1-maya.jpg'
import chasPamyatiNaKruzhkeBiblionik from '../images/photogallery/2019/chas-pamyati-na-kruzhke-biblionik.jpg'
import shlyapnayaVecherinka from '../images/photogallery/2019/shlyapnaya-vecherinka.jpg'
import shlyapnayaVecherinka2 from '../images/photogallery/2019/shlyapnaya-vecherinka-2.jpg'
import shlyapnayaVecherinka3 from '../images/photogallery/2019/shlyapnaya-vecherinka-3.jpg'
import shlyapnayaVecherinka4 from '../images/photogallery/2019/shlyapnaya-vecherinka-4.jpg'
import shlyapnayaVecherinka5 from '../images/photogallery/2019/shlyapnaya-vecherinka-5.jpg'
import bibliosumerki from '../images/photogallery/2019/bibliosumerki.jpg'
import bibliosumerki2 from '../images/photogallery/2019/bibliosumerki-2.jpg'
import bibliosumerki3 from '../images/photogallery/2019/bibliosumerki-3.jpg'
import detiNaViktorine from '../images/photogallery/2019/deti-na-viktorine.jpg'
import leto2019 from '../images/photogallery/2019/leto-2019.jpg'
import leto20192 from '../images/photogallery/2019/leto-2019-2.jpg'
import leto20193 from '../images/photogallery/2019/leto-2019-3.jpg'
import leto20194 from '../images/photogallery/2019/leto-2019-4.jpg'
import leto20195 from '../images/photogallery/2019/leto-2019-5.jpg'
import leto20196 from '../images/photogallery/2019/leto-2019-6.jpg'
import leto20197 from '../images/photogallery/2019/leto-2019-7.jpg'
import leto20198 from '../images/photogallery/2019/leto-2019-8.jpg'
import pohod906Karier from '../images/photogallery/2019/pohod-9.06-karier.jpg'
import pohod906Karier2 from '../images/photogallery/2019/pohod-9.06-karier-2.jpg'
import pohod906Karier3 from '../images/photogallery/2019/pohod-9.06-karier-3.jpg'
import pohod906Karier4 from '../images/photogallery/2019/pohod-9.06-karier-4.jpg'
import pohod206Besedka from '../images/photogallery/2019/pohod-2.06-besedka.jpg'
import pohod206SborMusora from '../images/photogallery/2019/pohod-2.06-sbor-musora.jpg'
import novogodneeZanyatieKruzhkaBiblionik from '../images/photogallery/2019/novogodnee-zanyatie-kruzhka-Biblionik.jpg'
import novogodneeZanyatieKruzhkaBiblionik2 from '../images/photogallery/2019/novogodnee-zanyatie-kruzhka-Biblionik-2.jpg'
import masterKlassPoIzgotovlenijuMyshekIzBumagiDomaUTaniSukhodoevoj from '../images/photogallery/2019/master-klass-po-izgotovleniju-myshek-iz-bumagi-doma-u-tani-sukhodoevoj.jpg'
import masterKlassPoIzgotovlenijuMyshekIzBumagiDomaUTaniSukhodoevoj2 from '../images/photogallery/2019/master-klass-po-izgotovleniju-myshek-iz-bumagi-doma-u-tani-sukhodoevoj-2.jpg'

import biblionik from '../images/photogallery/2018/biblionik.jpg'
import chtoGdeKogdaPoPravu from '../images/photogallery/2018/chto-gde-kogda-po-pravu.jpg'
import chtoGdeKogdaPoPravu2 from '../images/photogallery/2018/chto-gde-kogda-po-pravu-2.jpg'
import kvestVoVremyaNochiPottera from '../images/photogallery/2018/kvest-vo-vremya-nochi-pottera.jpg'
import garriPotter from '../images/photogallery/2018/garri-potter.jpg'
import vedma from '../images/photogallery/2018/vedma.jpg'
import horoshoSidim from '../images/photogallery/2018/horosho-sidim.jpg'
import katyaSupermama from '../images/photogallery/2018/katya-supermama.jpg'
import luchshieNaryady from '../images/photogallery/2018/luchshie-naryady.jpg'
import lukomorieK45letijuBiblioteki from '../images/photogallery/2018/lukomorie-k-45letiju-biblioteki.jpg'
import nochIskusstv from '../images/photogallery/2018/noch-iskusstv.jpg'
import tort from '../images/photogallery/2018/tort.jpg'
import otkryvaemYponiju from '../images/photogallery/2018/otkryvaem-yaponiju.jpg'
import novogodnijVecherNaSemejnomKlube from '../images/photogallery/2018/novogodnij-vecher-na-semejnom-klube.jpg'
import novogodnijVecherNaSemejnomKlube2 from '../images/photogallery/2018/novogodnij-vecher-na-semejnom-klube-2.jpg'
import novogodnijVecherNaSemejnomKlube3 from '../images/photogallery/2018/novogodnij-vecher-na-semejnom-klube-3.jpg'

import insideTheLibrary from '../images/photogallery/biblioteka-vnutri.jpg'
import insideTheLibrary2 from '../images/photogallery/biblioteka-vnutri-2.jpg'
import outsideTheLibrary from '../images/photogallery/biblioteka-snaruzhi.jpg'

export const photos2020 = [
  { src: rozhdestvenskijChasVeselykhZhatejDomaUTaniSukhodoevoj, alt: 'Один мальчик и четыре девочки сидят за столами и улыбаются', title: 'Рождественский час весёлых затей дома у Тани Суходоевой' },
  { src: vystavkaOPtitse2020Zhuravle, alt: 'Выставка с книгами и иллюстрациями о журавлях', title: 'Выставка о птице 2020 года — журавле' },
  { src: vystavkaProsmotrOChekhove, alt: 'На столе лежат раскрытые книги Чехова, а на стеллаже висит название выставки', title: 'Выставка-просмотр о Чехове' },
  { src: kruzhokBiblionikKJubilejuChekhova, alt: 'Дети детсадовского возраста обступили стол и рассматривают выставку о Чехове', title: 'Кружок «Библионик» к юбилею Чехова' },
  { src: besedaIViktorinaOChekhoveDlyaTaniSukhodoevoj, alt: 'Девочка сидит и рассматривает книгу, которую держит в руках', title: 'Беседа и викторина о Чехове для Тани Суходоевой' },
  { src: podarennyeGazetyOtPanishevojNV, alt: 'Газеты лежат на столе, вид сверху', title: 'Подаренные газеты от Панишевой Н.В.' },
  { src: plakatKMezhdunarodnomyDnjuKnigodareniya, alt: 'Нарисованный аист держит в клюве книгу, малыш сидит и читает книгу', title: 'Нарисованный плакат к Международному дню книгодарения' },
  { src: plakatKMezhdunarodnomyDnjuKnigodareniya2, alt: 'Плакат с вырезками из книг и стихом на книжную тему', title: 'Плакат с вырезками к Международному дню книгодарения' },
  { src: vystavkaAntarktidaKholodilnikZemli, alt: 'На столе лежат раскрытые книги с иллюстрациями на тему Антарктиды', title: 'Выставка «Антарктида — холодильник Земли»' },
  { src: JulyaKhlebnikovaSKnigojVRukakh, alt: 'Девочка в зимней одежде стоит у полок и держит в руках книгу', title: 'Юля Хлебникова с книгой в руках' },
]

export const photos2019 = [
  { src: semejnyjKlub29Marta, alt: 'Женщина и дети в костюмах, париках и шляпах в библиотеке на семейном клубе', title: 'Семейный клуб 29 марта' },
  { src: semejnyjKlub29Marta2, alt: 'Две женщины и дети в костюмах, париках и шляпах в библиотеке на семейном клубе', title: 'Семейный клуб 29 марта 2' },
  { src: chasZozhVperedPoDorogeZdoroviya, alt: 'Малыши из детского сада с воспитательницей стоят в библиотеке в верхней одежде', title: 'Час ЗОЖ вперёд по дороге здоровья' },
  { src: viktorinaNaDomuDlyaDevochkiSInvalidnostuj, alt: 'Девочка сидит за столом рядом с женщиной, обе смотрят в камеру', title: 'Викторина на дому для девочки с инвалидностью' },
  { src: vystavkaTeatrIBiblioteka, alt: 'Выставка с иллюстрациями и статьями на театральную тему «Театр и библиотека»', title: 'Выставка «Театр и библиотека»' },
  { src: detsadovtsyVMaskah, alt: 'Детсадовцы и библиотекарь в бумажных масках', title: 'Детсадовцы в масках на кружке «Библионик»' },
  { src: bibliochuchelo, alt: 'Библиочучело из книг', title: 'Библиочучело для Библионочи' },
  { src: bibliopiknik1Maya, alt: 'Пять подростков стоят на улице', title: 'Библиопикник 1 мая' },
  { src: chasPamyatiNaKruzhkeBiblionik, alt: 'Выставка фотографий и рисунков на тему войны «Про тех, кто сражался и победил»', title: 'Час памяти на кружке «Библионик»' },
  { src: shlyapnayaVecherinka, alt: 'Шляпы из бумаги лежат на столе', title: 'Шляпная вечеринка' },
  { src: shlyapnayaVecherinka2, alt: 'Дети в разных бумажных шляпах сидят на стульях в библиотеке', title: 'Шляпная вечеринка 2' },
  { src: shlyapnayaVecherinka3, alt: 'Дети в разных бумажных шляпах сидят на стульях в библиотеке', title: 'Шляпная вечеринка 3' },
  { src: shlyapnayaVecherinka4, alt: 'Дети в разных бумажных шляпах сидят на стульях в библиотеке', title: 'Шляпная вечеринка 4' },
  { src: shlyapnayaVecherinka5, alt: 'Дети синхронно танцуют', title: 'Шляпная вечеринка 5' },
  { src: bibliosumerki, alt: 'Большой дуб, русалка на ветвях, кот учёный и другие сказочные персонажи из бумаги', title: 'Библиосумерки' },
  { src: bibliosumerki2, alt: 'Много детей разных возростов сидят на стульях в библиотеке', title: 'Библиосумерки 2' },
  { src: bibliosumerki3, alt: 'Много детей разных возростов сидят на стульях в библиотеке', title: 'Библиосумерки 3' },
  { src: detiNaViktorine, alt: 'Подростки сидят за столами в библиотеке', title: 'Дети на викторине' },
  { src: leto2019, alt: 'Дети сидят на стульях в библиотеке', title: 'Лeто 2019' },
  { src: leto20192, alt: 'Два мальчика, позади них флаг РФ', title: 'Лeто 2019 2' },
  { src: leto20193, alt: 'Выставка на тему экологии «Экономия и бережливость»', title: 'Лeто 2019 3' },
  { src: leto20194, alt: 'Три девочки и один мальчик разных возрастов сидят на столом', title: 'Лeто 2019 4' },
  { src: leto20195, alt: 'Подростки сидят за столами друг напротив друга', title: 'Лeто 2019 5' },
  { src: leto20196, alt: 'Детсадовцы сидят на стульях в библиотеке', title: 'Лeто 2019 6' },
  { src: leto20197, alt: 'Выставка из статей и иллюстраций «Флаг у нас прекрасный — сине-бело-красный»', title: 'Лeто 2019 7' },
  { src: leto20198, alt: 'Детсадовцы и воспитательница стоят на фоне флага РФ', title: 'Лeто 2019 8' },
  { src: pohod906Karier, alt: 'Дети и взрослые (снято со спины) идут в поход по дороге среди деревьев', title: 'Поход 9 июня, карьер' },
  { src: pohod906Karier2, alt: 'Мальчик стоит у карьера', title: 'Поход 9 июня, карьер 2' },
  { src: pohod906Karier3, alt: 'Мальчик бегает в карьере, на заднем плане стоят дети', title: 'Поход 9 июня, карьер 3' },
  { src: pohod906Karier4, alt: 'Дети возвращаются домой из похода на карьер', title: 'Поход 9 июня, карьер 4' },
  { src: pohod206Besedka, alt: 'Летний поход детей и взрослых на лесную поляну с беседкой', title: 'Поход 2 июня, беседка' },
  { src: pohod206SborMusora, alt: 'Пакеты с мусором, который был собран рядом с лесной беседкой', title: 'Поход 2 июня, сбор мусора библиотекарем' },
  { src: novogodneeZanyatieKruzhkaBiblionik, alt: 'Детсадовцы и воспитательница сидят в библиотеке в верхней одежде', title: 'Новогоднее занятие кружка Библионик' },
  { src: novogodneeZanyatieKruzhkaBiblionik2, alt: 'Детсадовцы и воспитательница сидят в библиотеке в верхней одежде', title: 'Новогоднее занятие кружка Библионик 2' },
  { src: masterKlassPoIzgotovlenijuMyshekIzBumagiDomaUTaniSukhodoevoj, alt: 'Две девочки и мальчик разных возрастов мастерят поделки из бумаги', title: 'Мастер-класс по изготовлению мышек из бумаги дома у Тани Суходоевой' },
  { src: masterKlassPoIzgotovlenijuMyshekIzBumagiDomaUTaniSukhodoevoj2, alt: 'Мышки из цветной бумаги', title: 'Мастер-класс по изготовлению мышек из бумаги дома у Тани Суходоевой 2' },
]

export const photos2018 = [
  { src: biblionik, alt: 'Детсадовцы сидят на стульях и стоят в библиотеке', title: 'Библионик' },
  { src: chtoGdeKogdaPoPravu, alt: 'Дети сидят на стульях, позади них стоит библиотекарь', title: 'Что, где, когда по праву' },
  { src: chtoGdeKogdaPoPravu2, alt: 'Дети сидят на стульях', title: 'Что, где, когда по праву 2' },
  { src: kvestVoVremyaNochiPottera, alt: 'Дети сидят за столом в библиотеке и пьют чай', title: 'Квест во время ночи Поттера' },
  { src: garriPotter, alt: 'Мальчик в очках и с волшебной палочкой', title: 'Гарри Поттер' },
  { src: vedma, alt: 'Библиотекарь в шляпе ведьмы и черепом в руках', title: 'Ведьма' },
  { src: horoshoSidim, alt: 'Три женщины и девочка в разноцветных халатах', title: 'Хороши сидим' },
  { src: katyaSupermama, alt: 'Женщина стоит у полок с книгами, сзади надпись «Супермама»', title: 'Катя — супермама' },
  { src: luchshieNaryady, alt: 'Женщины и дети стоят у полок с книгами', title: 'Лучшие наряды' },
  { src: lukomorieK45letijuBiblioteki, alt: 'Целая стена, украшенная сказочными персонажами из бумаги', title: 'Лукоморье к 45-летию библиотеки' },
  { src: nochIskusstv, alt: 'Дети смеются и играют в игру в библиотеке', title: 'Ночь искусств' },
  { src: tort, alt: 'Стеллажи с книгами и торт из книг', title: 'Торт' },
  { src: otkryvaemYponiju, alt: 'Выставка с иллюстрациями и бумажными журавлями «Открываем Японию»', title: 'Открываем Японию' },
  { src: novogodnijVecherNaSemejnomKlube, alt: 'Дети и взрослые на новогоднем семейном клубе стоят и сидят у книжных полок в костюмах', title: 'Новогодний вечер на семейном клубе' },
  { src: novogodnijVecherNaSemejnomKlube2, alt: 'Дети и взрослые на новогоднем семейном клубе стоят у книжных полок в костюмах', title: 'Новогодний вечер на семейном клубе 2' },
  { src: novogodnijVecherNaSemejnomKlube3, alt: 'Мама и сын на новогоднем семейном клубе стоят у книжных полок и смеются', title: 'Новогодний вечер на семейном клубе 3' },
]

export const libraryPhotos = [
  { src: insideTheLibrary, alt: 'Краснопольская библиотека внутри' },
  { src: insideTheLibrary2, alt: 'Краснопольская библиотека внутри 2' },
  { src: outsideTheLibrary, alt: 'Краснопольская библиотека снаружи' },
]
